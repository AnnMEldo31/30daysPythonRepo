...
def print_show_info(tv_show) :
  title, seasons, release_year = tv_show.values()
  print(f"{title} ({release_year}) - {seasons} seasons")
...
def test_palindrome(strng) :
  tester = strng.strip().lower()
  if tester == tester[::-1] :
    print(f"\"{strng}\" is a Palindrome")
  else :
    print(f"\"{strng}\" is not a Palindrome")

test_palindrome("Hannah")
test_palindrome("True Detective")